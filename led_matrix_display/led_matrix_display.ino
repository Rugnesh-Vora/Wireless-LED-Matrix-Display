...
{
  // this function displays symbols
  i = 0;// resets the counter fot the 4017
  for (int g = 0; g < 6; g++) //resets the the long int where word goes
    scrolling_word[g] = 0;
  for (int x = 0; x < num_patterns; x++)
  {
    //main loop, goes over symbols
    ...
    for (int z = 0; z < 6; z++)
    {
      ...
      for (int t = 0; t < delay_langth; t++)
      {
        ...
        for (int y = 0; y < 6; y++)
        {
          ...
          latchOn();
          delayMicroseconds(1500);//waiting a bit
          latchOff();
...
//display_word(1,patterns,43,15);// calls for the display_pattern function and says that int loop = 15(if do more loop the pattern whould scrrol slower).
